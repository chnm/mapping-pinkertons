...
- Ensure `vizType` in frontmatter matches the JavaScript filename exactly
- Verify the JS file is in `/themes/pinkertons/static/js/visualizations/`
- Check that you're exporting `createVisualization` function

### Module loading errors
- Filename must match `vizType` parameter exactly (case-sensitive)
- JS file must be in the correct directory
- Must use ES6 module syntax (`import`/`export`)
- Check browser Network tab to see if the file loads

### Data isn't loading
- Make sure the API server is running on `localhost:8090`
- Check network tab in browser dev tools
- Verify the data structure matches your expectations

### Chart looks wrong
- Check that field names match your data structure
- Verify data transformations are working correctly
- Review Observable Plot documentation for your mark type
- Use `console.log(data)` to inspect the data in your function

## Advanced: Creating Custom Visualization Types

For more complex visualizations:

1. Consider combining multiple marks (bars + lines, dots + areas, etc.)
2. Use faceting to create small multiples
3. Add interaction with `Plot.pointer()` or custom event listeners
4. Create derived data with `Plot.map()` or `Plot.bin()`

See the Observable Plot documentation for advanced techniques.

## Questions or Issues?

If you encounter problems or need help:
1. Check the Observable Plot documentation
2. Review existing visualization functions as examples
3. Test with a simplified version of your data
4. Use `console.log()` to debug data transformations
